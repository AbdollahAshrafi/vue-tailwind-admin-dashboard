        ...
        <div>
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            <div v-for="field in fields" :key="field.key">
              <p class="mb-2 text-sm leading-normal text-gray-500 dark:text-gray-400">{{ field.label }}</p>
              <p class="text-md font-medium text-gray-800 dark:text-white/90">
                <span v-if="field.type === 'amount'">{{ formatAmount(data[field.key]) }}</span>
                <span v-else-if="field.type === 'status'">
                  <span :class="getStatusClass(data[field.key])" class="px-2 py-1 text-sm rounded-full">
                    {{ data[field.key] }}
                  </span>
                </span>
